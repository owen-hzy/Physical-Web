...
function startBridge () {
        ...
        connection.onopen = function (session) {
            ...
            arduino.on('analogChange', function (e) {
                if (monitored_pins[e.pin]) {
                    ...
                    session.publish(make_uri("on_analog_changed"), [e.pin, e.value]);
                }
            });
    /*
    ...
    */
			...
			function sysex_write (args) {
				var address = args[0];
				var mode = args[1];
				var value = args[2];

				if (debug) {
					console.log("sysex_write", address, mode, value);
				}

				try {
					arduino.sysex(0x76, [address, mode, value]);
				} catch (e) {
					console.log(e);
					throw e;
				}
			}
			reg_requests.push(session.register(make_uri('sysex_write'), sysex_write));
