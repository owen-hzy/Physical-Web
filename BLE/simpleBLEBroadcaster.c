...
#define URLCFG_CHAR_URI_DATA_DEFAULT          "http://goo.gl/uYT8wu"
...
void SimpleBLEBroadcaster_Init( uint8 task_id )
{
  ...
  // Setup the GAP Broadcaster Role Profile
  {
    ...
    // Init URL frame
    uint8 temp;
    temp = SimpleEddystoneBeacon_encodeURL(URLCFG_CHAR_URI_DATA_DEFAULT, eddystoneURL.encodedURL);
    eddystoneURL.frameType = EDDYSTONE_FRAME_TYPE_URL;
    eddystoneURL.txPower = TX_POWER_MODE_LOW;
    uint8 frameSize;
    eddystoneAdv.length = EDDYSTONE_SVC_DATA_OVERHEAD_LEN;
    frameSize = sizeof(eddystoneURL_t) - EDDYSTONE_MAX_URL_LEN + temp;
    memcpy((uint8 *) &eddystoneAdv.frame, (uint8 *) &eddystoneURL, frameSize);
    ...
    GAPRole_SetParameter( GAPROLE_ADVERT_DATA, EDDYSTONE_FRAME_OVERHEAD_LEN + eddystoneAdv.length, &eddystoneAdv );
